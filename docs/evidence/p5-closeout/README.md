...
## New Water preview — NOT APPROVED / NOT STARTED
...
Only the initial Opt is ready for owner approval:
...
After approval, execute just this Opt once. Preserve its actual stdout and final
XYZ; restart the CLI, verify one physical Opt start and unchanged command-replay
results, then present the resulting Freq action for separate approval. SP also
requires separate approval. Earlier bounded authorizations are not reused for
this new chain. No merge or Owner acceptance is inferred from this preview.
