...
is therefore a no-op for an already registered effect.
...
use the injected UTC clock. Retry delays are deterministic (1, 2, 4, 8, and
16 seconds, capped at 60 seconds) with a maximum of five attempts. Error
messages are bounded safe summaries and never contain tracebacks.
