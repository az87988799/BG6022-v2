...
def test_migration_is_idempotent_across_close_and_reopen(tmp_path) -> None:
    ...
    try:
        ...
        assert apply_migrations(reopened, clock=clock) == DEFAULT_MIGRATIONS[-1].version
...
def test_registry_gaps_and_future_database_versions_fail_closed(tmp_path) -> None:
    ...
    try:
        ...
        connection.execute(
            ...
            "VALUES (?, 'future', ?, '2026-09-04T00:00:00.000000Z')",
            (DEFAULT_MIGRATIONS[-1].version + 1, "1" * 64),
        )
        with pytest.raises(MigrationVersionError):
            apply_migrations(connection)
    finally:
...
def test_v2_upgrades_existing_v1_data_and_backfills_immutable_effect_metadata(tmp_path) -> None:
    ...
    try:
        ...
        assert apply_migrations(connection) == DEFAULT_MIGRATIONS[-1].version
        ...
        assert [
            row[0]
            for row in connection.execute("SELECT version FROM schema_migrations ORDER BY version")
        ] == list(range(1, DEFAULT_MIGRATIONS[-1].version + 1))
